...
$profileSheet->setTitle('School Clients Information');
...
$profile_sql = "SELECT c.client_name, 
                ...
                WHERE c.user_id = ? AND s.sector_name = 'School'
...
$evalSheet->setTitle('School Clients Evaluation');

// Fetch evaluated client evaluation data for School sector
$eval_sql = "SELECT c.client_name, cr.criteria_criterion, r.rating_score, r.rating_notes, e.evaluation_rating, e.evaluation_result, e.evaluation_date
             ...
             WHERE c.user_id = ? AND c.sector_id = (SELECT sector_id FROM tbl_sector WHERE sector_name = 'School')
...
header('Content-Disposition: attachment;filename="School_Clients_Export_' . date('Y-m-d') . '.xlsx"');
