...
$profileSheet->setTitle('Evaluated Clients Information');
...
$headerStyle = [
    ...
    'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_MEDIUM]] // Thicker border for headers
];
$profileSheet->getStyle('A1:I1')->applyFromArray($headerStyle);

// Populate profile data with spacing between clients
$row = 2;
$currentClient = null;
...
$evalSheet->setTitle('Evaluated Clients Evaluation');

// Fetch evaluated client evaluation data
...
while ($eval_data = $eval_result->fetch_assoc()) {
    if ($currentClient !== $eval_data['client_name']) {
        ...
        $evalSheet->setCellValue('A' . $row, $eval_data['client_name'] ?: 'Not Available');
        $evalSheet->setCellValue('B' . $row, $eval_data['evaluation_rating'] ?: 'Not Available');
        $evalSheet->setCellValue('C' . $row, $eval_data['evaluation_result'] ?: 'Not Available');
        $evalSheet->setCellValue('D' . $row, $eval_data['evaluation_date'] ?: 'Not Available');
    }
    
    $evalSheet->setCellValue('E' . $row, $eval_data['criteria_criterion'] ?: 'Not Available');
    $evalSheet->setCellValue('F' . $row, $eval_data['rating_score'] ?: 'Not Available');
    $evalSheet->setCellValue('G' . $row, $eval_data['rating_notes'] ?: 'Not Available');

    // Apply thicker borders to the row
    $evalSheet->getStyle("A$row:G$row")->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_MEDIUM);
...
foreach ($evalSheet->getColumnIterator() as $column) {
    $evalSheet->getColumnDimension($column->getColumnIndex())->setAutoSize(true);
}
...
header('Content-Disposition: attachment;filename="Evaluated_Clients_Export_' . date('Y-m-d') . '.xlsx"');
