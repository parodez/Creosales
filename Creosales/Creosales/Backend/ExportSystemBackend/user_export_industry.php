...
$profileSheet->setTitle('Industry Clients Information');
...
$profile_sql = "SELECT c.client_name, 
                ...
                WHERE c.user_id = ? AND s.sector_name = 'Industry'
...
$evalSheet->setTitle('Industry Clients Evaluation');

// Fetch evaluated client evaluation data for Industry sector
$eval_sql = "SELECT c.client_name, cr.criteria_criterion, r.rating_score, r.rating_notes, e.evaluation_rating, e.evaluation_result, e.evaluation_date
             ...
             WHERE c.user_id = ? AND c.sector_id = (SELECT sector_id FROM tbl_sector WHERE sector_name = 'Industry')
...
header('Content-Disposition: attachment;filename="Industry_Clients_Export_' . date('Y-m-d') . '.xlsx"');
