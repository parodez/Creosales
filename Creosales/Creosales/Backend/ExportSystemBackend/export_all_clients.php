...
$profileSheet->setTitle('All Clients Information');

// Fetch all client profile data with contact persons
...
$evalSheet->setTitle('All Clients Evaluation');
...
// Ensure the overall rating and rating columns accept one decimal place
$evalSheet->getStyle('B2:B' . ($row - 1))->getNumberFormat()->setFormatCode('0.0');
$evalSheet->getStyle('F2:F' . ($row - 1))->getNumberFormat()->setFormatCode('0.0');
...
header('Content-Disposition: attachment;filename="All_Clients_Export_' . date('Y-m-d') . '.xlsx"');
