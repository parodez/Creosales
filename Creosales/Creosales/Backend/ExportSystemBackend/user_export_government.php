...
$profileSheet->setTitle('Government Clients Information');
...
$profile_sql = "SELECT c.client_name, 
                ...
                WHERE c.user_id = ? AND s.sector_name = 'Government'
...
$evalSheet->setTitle('Government Clients Evaluation');

// Fetch evaluated client evaluation data for Government sector
$eval_sql = "SELECT c.client_name, cr.criteria_criterion, r.rating_score, r.rating_notes, e.evaluation_rating, e.evaluation_result, e.evaluation_date
             ...
             WHERE c.user_id = ? AND c.sector_id = (SELECT sector_id FROM tbl_sector WHERE sector_name = 'Government')
...
header('Content-Disposition: attachment;filename="Government_Clients_Export_' . date('Y-m-d') . '.xlsx"');
